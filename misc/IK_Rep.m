function [OutputStatement,OsimFileName]=IK(DataDirectory, FunctionDirectory,TrialName,Specifier,OsimFileName,timeRange)
    ...
    xmlDoc = xmlread([DataDirectory '\Setup_IK.xml']);
    %%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if isempty(FunctionDirectory)==1
         Command = ['IK -S Setup_IK.xml']; %IK.exe is in the system path
    else
         Command = [FunctionDirectory 'IK -S Setup_IK.xml']; %Ik.exe is not in the system path or you wish to specify which one
    end
    %% Run IK.exe
    system(Command);
