...
%% go to the folder with all the setup files
     [setupName,SetupDirectory]=uigetfile('*Setup_IK.xml','SetUp folder?',ModelDirectory,'MultiSelect','off');
     cd(SetupDirectory)
%%
...
%% Copy the osim file into the directory
  copyfile([SetupDirectory 'Setup_IK.xml'],[DataDirectory 'Setup_IK.xml']);
  copyfile([SetupDirectory 'IK_Tasks.xml'],[DataDirectory 'IK_Tasks.xml']);
  cd(DataDirectory)
%%
...
%% Delete temp files
 delete([DataDirectory 'Setup_IK.xml']);
  delete([DataDirectory 'IK_Tasks.xml']);
