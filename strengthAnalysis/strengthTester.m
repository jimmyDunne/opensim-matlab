...
%%
muscles.quad = {'vas_med_r' 'rect_fem_r' 'vas_int_r' 'vas_lat_r'};

muscles.hipabd = {'glut_max1_r'...
            'glut_med1_r' 'glut_med2_r' 'glut_med3_r'...
            'glut_min1_r' 'glut_min2_r' 'glut_min3_r'...
            'peri_r' 'sar_r' 'tfl_r'};

muscles.planterFlex = {'flex_dig_r' 'flex_hal_r'...
             'lat_gas_r' 'med_gas_r' ...
             'per_brev_r' 'per_long_r'...
             'soleus_r' 'tib_post_r'};

muscles.dorsiFlex = {'ext_dig_r' 'ext_hal_r'...
            'per_tert_r' 'tib_ant_r'};
       
muscles.hamstrings = {'bifemsh_r' 'bifemlh_r'...
           'semiten_r' 'semimem_r'};
...
%%
groupNames = fieldnames(muscles);
nGroups = length(groupNames);

for i = 1 : nGroups

    % Results folder name
    resultsFolder = ['ResultsCMC' char(groupNames{i})];

    if exist([pathname '\' resultsFolder])
        continue
    else
        mkdir([pathname '\' resultsFolder])
    end
    
    muscleList = muscles.(groupNames{i});
    
    % run the 100% trial
    display(['mCapacity; ' num2str(100)]);
    [t q] = runCMCtool(pathname,resultsFolder,muscleList,100,cmcName,modelName);
    ...
    while abs(stepSize(1)-stepSize(2)) > 1 
        ...
        if nSteps == 1
            ...
            [t_n q_n] = runCMCtool(pathname,resultsFolder,muscleList,currentValue,cmcName,modelName);
